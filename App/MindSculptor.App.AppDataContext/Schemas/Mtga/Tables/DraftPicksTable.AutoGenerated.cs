...
namespace MindSculptor.App.AppDataContext.Schemas.Mtga.Tables
{
    ...
    {
        private DraftPicksTable(DatabaseContext dataContext) : base(dataContext, "Mtga", "DraftPicks")
        {
        }

        internal static DraftPicksTable Create(DatabaseContext dataContext)
        {
        ...
        {
            return Context.Execute(command => NewRecord(command, Guid.NewGuid(), eventEntryId, packNumber, pickNumber, isFifthCopy));
        }
        ...
        {
            return await Context.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, Guid.NewGuid(), eventEntryId, packNumber, pickNumber, isFifthCopy, cancellationToken), cancellationToken).ConfigureAwait(false);
        }

        public DraftPickRecord NewRecord(EventEntryRecord eventEntryRecord, int packNumber, int pickNumber, bool isFifthCopy)
        {
            return Context.Execute(command => NewRecord(command, Guid.NewGuid(), eventEntryRecord.Id, packNumber, pickNumber, isFifthCopy));
        }
        ...
        {
            return await Context.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, Guid.NewGuid(), eventEntryRecord.Id, packNumber, pickNumber, isFifthCopy, cancellationToken), cancellationToken).ConfigureAwait(false);
        }
        ...
        {
            var newRecord = DraftPickRecord.Create(Context, this, id, eventEntryId, packNumber, pickNumber, isFifthCopy);
        ...
        protected override DraftPickRecord MapRecordFromDataReader(DbDataReader dbDataReader)
        {
            ...
            return DraftPickRecord.Create(Context, this, id, eventEntryId, packNumber, pickNumber, isFifthCopy);
        }
    }
}