...
namespace MindSculptor.App.AppDataContext.Schemas.Mtga.Tables
{
    ...
    {
        private DigitalCardsTable(DatabaseContext dataContext) : base(dataContext, "Mtga", "DigitalCards")
        {
        }

        internal static DigitalCardsTable Create(DatabaseContext dataContext)
        {
            return new DigitalCardsTable(dataContext);
        }

        public DigitalCardRecord NewRecord(Guid id, int mtgaCardId)
        {
            return Context.Execute(command => NewRecord(command, id, mtgaCardId));
        }

        public async Task<DigitalCardRecord> NewRecordAsync(Guid id, int mtgaCardId, CancellationToken cancellationToken = default)
        {
            return await Context.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, id, mtgaCardId, cancellationToken), cancellationToken).ConfigureAwait(false);
        }

        public DigitalCardRecord NewRecord(BasePrintingRecord basePrintingRecord, int mtgaCardId)
        {
            return Context.Execute(command => NewRecord(command, basePrintingRecord.Id, mtgaCardId));
        }
        ...
        {
            return await Context.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, basePrintingRecord.Id, mtgaCardId, cancellationToken), cancellationToken).ConfigureAwait(false);
        }

        private DigitalCardRecord NewRecord(DbCommand command, Guid id, int mtgaCardId)
        {
            var newRecord = DigitalCardRecord.Create(Context, this, id, mtgaCardId);
        ...
        protected override DigitalCardRecord MapRecordFromDataReader(DbDataReader dbDataReader)
        {
            ...
            return DigitalCardRecord.Create(Context, this, id, mtgaCardId);
        }
    }
}