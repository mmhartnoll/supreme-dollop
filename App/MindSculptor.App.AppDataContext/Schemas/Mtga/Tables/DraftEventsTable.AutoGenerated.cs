...
namespace MindSculptor.App.AppDataContext.Schemas.Mtga.Tables
{
    ...
    {
        private DraftEventsTable(DatabaseContext databaseContext) : base(databaseContext, "Mtga", "DraftEvents")
        {
        }

        internal static DraftEventsTable Create(DatabaseContext databaseContext)
        {
            return new DraftEventsTable(databaseContext);
        }

        public DraftEventRecord NewRecord(Guid eventId, Guid setId, string draftType)
        {
            return DatabaseContext.Execute(command => NewRecord(command, eventId, setId, draftType));
        }
        ...
        {
            return await DatabaseContext.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, eventId, setId, draftType, cancellationToken), cancellationToken).ConfigureAwait(false);
        }

        public DraftEventRecord NewRecord(EventRecord eventRecord, SetRecord setRecord, string draftType)
        {
            return DatabaseContext.Execute(command => NewRecord(command, eventRecord.Id, setRecord.Id, draftType));
        }
        ...
        {
            return await DatabaseContext.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, eventRecord.Id, setRecord.Id, draftType, cancellationToken), cancellationToken).ConfigureAwait(false);
        }

        private DraftEventRecord NewRecord(DbCommand command, Guid eventId, Guid setId, string draftType)
        {
            var newRecord = DraftEventRecord.Create(DatabaseContext, this, eventId, setId, draftType);
        ...
        protected override DraftEventRecord MapRecordFromDataReader(DbDataReader dbDataReader)
        {
            ...
            return DraftEventRecord.Create(DatabaseContext, this, eventId, setId, draftType);
        }
    }
}