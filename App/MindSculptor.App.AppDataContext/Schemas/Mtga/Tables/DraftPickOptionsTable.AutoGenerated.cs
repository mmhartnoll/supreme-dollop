...
namespace MindSculptor.App.AppDataContext.Schemas.Mtga.Tables
{
    ...
    {
        private DraftPickOptionsTable(DatabaseContext databaseContext) : base(databaseContext, "Mtga", "DraftPickOptions")
        {
        }

        internal static DraftPickOptionsTable Create(DatabaseContext databaseContext)
        {
            return new DraftPickOptionsTable(databaseContext);
        }
        ...
        {
            return DatabaseContext.Execute(command => NewRecord(command, Guid.NewGuid(), eventId, eventEntryId, digitalCardId, packNumber, pickNumber, ordinal));
        }
        ...
        {
            return await DatabaseContext.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, Guid.NewGuid(), eventId, eventEntryId, digitalCardId, packNumber, pickNumber, ordinal, cancellationToken), cancellationToken).ConfigureAwait(false);
        }
        ...
        {
            return DatabaseContext.Execute(command => NewRecord(command, Guid.NewGuid(), draftEventRecord.EventId, eventEntryRecord.Id, digitalCardRecord.Id, packNumber, pickNumber, ordinal));
        }
        ...
        {
            return await DatabaseContext.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, Guid.NewGuid(), draftEventRecord.EventId, eventEntryRecord.Id, digitalCardRecord.Id, packNumber, pickNumber, ordinal, cancellationToken), cancellationToken).ConfigureAwait(false);
        }
        ...
        {
            var newRecord = DraftPickOptionRecord.Create(DatabaseContext, this, id, eventId, eventEntryId, digitalCardId, packNumber, pickNumber, ordinal);
        ...
        protected override DraftPickOptionRecord MapRecordFromDataReader(DbDataReader dbDataReader)
        {
            ...
            return DraftPickOptionRecord.Create(DatabaseContext, this, id, eventId, eventEntryId, digitalCardId, packNumber, pickNumber, ordinal);
        }
    }
}