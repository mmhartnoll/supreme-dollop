...
namespace MindSculptor.App.AppDataContext.Schemas.Cards.Tables
{
    ...
    {
        private OracleTextTable(DatabaseContext databaseContext) : base(databaseContext, "Cards", "OracleText")
        {
        }

        internal static OracleTextTable Create(DatabaseContext databaseContext)
        {
            return new OracleTextTable(databaseContext);
        }

        public OracleTextRecord NewRecord(Guid faceId, string value)
        {
            return DatabaseContext.Execute(command => NewRecord(command, faceId, value));
        }

        public async Task<OracleTextRecord> NewRecordAsync(Guid faceId, string value, CancellationToken cancellationToken = default)
        {
            return await DatabaseContext.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, faceId, value, cancellationToken), cancellationToken).ConfigureAwait(false);
        }

        public OracleTextRecord NewRecord(FaceRecord faceRecord, string value)
        {
            return DatabaseContext.Execute(command => NewRecord(command, faceRecord.Id, value));
        }
        ...
        {
            return await DatabaseContext.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, faceRecord.Id, value, cancellationToken), cancellationToken).ConfigureAwait(false);
        }

        private OracleTextRecord NewRecord(DbCommand command, Guid faceId, string value)
        {
            var newRecord = OracleTextRecord.Create(DatabaseContext, this, faceId, value);
        ...
        protected override OracleTextRecord MapRecordFromDataReader(DbDataReader dbDataReader)
        {
            ...
            return OracleTextRecord.Create(DatabaseContext, this, faceId, value);
        }
    }
}