...
namespace MindSculptor.App.AppDataContext.Schemas.Cards.Tables
{
    ...
    {
        private LoyaltyTable(DatabaseContext dataContext) : base(dataContext, "Cards", "Loyalty")
        {
        }

        internal static LoyaltyTable Create(DatabaseContext dataContext)
        {
        ...
        {
            return Context.Execute(command => NewRecord(command, faceId, baseLoyalty, loyaltyFormat));
        }
        ...
        {
            return await Context.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, faceId, baseLoyalty, loyaltyFormat, cancellationToken), cancellationToken).ConfigureAwait(false);
        }

        public LoyaltyRecord NewRecord(FaceRecord faceRecord, int baseLoyalty, string loyaltyFormat)
        {
            return Context.Execute(command => NewRecord(command, faceRecord.Id, baseLoyalty, loyaltyFormat));
        }
        ...
        {
            return await Context.ExecuteAsync((command, cancellationToken) => NewRecordAsync(command, faceRecord.Id, baseLoyalty, loyaltyFormat, cancellationToken), cancellationToken).ConfigureAwait(false);
        }

        private LoyaltyRecord NewRecord(DbCommand command, Guid faceId, int baseLoyalty, string loyaltyFormat)
        {
            var newRecord = LoyaltyRecord.Create(Context, this, faceId, baseLoyalty, loyaltyFormat);
        ...
        protected override LoyaltyRecord MapRecordFromDataReader(DbDataReader dbDataReader)
        {
            ...
            return LoyaltyRecord.Create(Context, this, faceId, baseLoyalty, loyaltyFormat);
        }
    }
}